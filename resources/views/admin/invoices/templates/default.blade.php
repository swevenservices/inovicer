     ...
     text-align: left!important; 
               ...
               @if($invoice->type =="invoice")
                ...
                <div class="row">
                    ...
                    @if($invoice->type == "qoute")
                    <div class="col-sm-3 col-4 text-center" style="background-color: #df7626; margin:auto">
                        <div class="mt-2">
                    ...
                    <div class="col-sm-3 col-4 text-center" style="background-color: #252968; margin:auto">
                        <div class="mt-2">
                    ...
                    @else
                    <div class="col-sm-2 col-4 text-center" style="background-color: #df7626">
                        <div class="mt-2">
                            <i class="fa fa-file-text-o fa-2x" style="color: white"></i>
                        </div>
                        <div class="text-grey-m2">
                    ...
                    <div class="col-sm-2 col-4 text-center" style="background-color: #252968">
                        <div class="mt-2">
                            <i class="fa fa-calendar fa-2x" style="color: white"></i>
                        </div>
                        <div class="text-grey-m2">
                    ...
                    <div class="col-sm-2 col-4 text-center" style="background-color: #df7626">
                        <div class="mt-2">
                            <i class="fa fa-usd fa-2x" style="color: white"></i>
                        </div>
                        <div class="text-grey-m2">
                            <div class="my-1">
                                DUE DATE
                    ...
                    <div class="col-sm-6 text-right" style="position: relative; right: 10px ">
                        ...
                        <div class="text-grey-m2 mt-2">
                            <div class="my-1">
                                <p disabled  style=" padding:10px;float:right ; color:black; height: auto;background-color: #80808021; width: 80%;border: none;text-align: center;font-size: 19px;" >{{$invoice->customer_name}}</p>
                            </div>
                        
                            <div class="my-1">
                                <p disabled  style="padding:10px;float:right ;height: auto; color:black; background-color: #80808021; width: 80%;border: none;text-align: center;font-size: 17px;" >{!! $invoice->complete_address !!}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-2 ">
                    <div class="row text-600 text-white " style="height: 40px">
                       <div class="col-1 col-sm-1" style="background-color: #252968;padding: 5px;"> S.N</div>
                        <div class="col-8 col-sm-4" style="background-color: #252968;padding: 10px;"> DESCRIPTION</div>
                        <div class="d-none d-sm-block col-4 col-sm-2" style="background-color: #df7626;padding: 10px;">QUANTITY</div>
                       
                        <div class="d-none d-sm-block col-sm-2" style="background-color: #df7626;padding: 10px;">UNIT PRICE</div>
                        
                        <div class="d-none d-sm-block col-sm-3" style="background-color: #df7626;padding: 10px;">TOTAL</div>
                    </div>
                    <div class="text-95 text-secondary-d3">
                        ...
                        @foreach($invoice->invoiceInvoiceItems as $key => $item)
                            <div class="row mb-2 mb-sm-0 py-25 {{$counter % 2? 'bcr-1': 'brc-default-l2' }}">
                                ...
                                <div class="col-8 col-sm-4 text-center">{{$item->name}}<br>
                                <span class="text-left" style="font-size: 12px;">{!! $item->description !!}</span></div>
                                <div class="d-none d-sm-block col-2">{{$item->qty}}</div>
                                <div class="d-none d-sm-block col-2 text-95"> @if($invoice->currency) {{$invoice->currency->symbol}} @endif {{ number_format( $item->total ,2)}}</div>
                 
                                <div class="col-3 text-secondary-d2">  @if($invoice->currency) {{$invoice->currency->symbol}} @endif {{ number_format($item->qty * $item->total ,2)}}</div>
                            </div>
                   @php $total += $item->qty * $item->total;
                   $counter++; @endphp
                   @endforeach
                   @endif
                @if($invoice->vat)
                        ...
                        <div class="col-12 col-sm-5 text-grey text-90 order-first order-sm-last">
                            <div class="row" >
                                ...
                                <div class="col-6" style="background-color: #df7626; color: white">
                                    <span class="text-120 ">  @if($invoice->currency) {{$invoice->currency->symbol}} @endif {{ number_format($total ,2)}} </span>
                                </div>
                                <hr style="width: 100%"/>
                            </div>
                            ...
                            <div class="row ">
                                <div class="col-6 text-left" style="color: #252968;">
                                    <b> VAT {{$invoice->vat ? '( '.$invoice->vat. ' % )' :  ''}} </b>
                                </div>
                                <div class="col-6" style="background-color: #df7626; color: white">
                                    <span class="text-120 ">   @if($invoice->currency) {{  $invoice->currency->symbol}} @endif {{ number_format($vat, 2)}} </span>
                                </div>
                                <hr style="width: 100%"/>
                            </div>
                            @if($invoice->discount)
                                <div class="row ">
                                    <div class="col-6 text-left" style="color: #252968;">
                                        <b> DISCOUNT  {{ $invoice->discount_type =="fixed" ? '( AED )' :'( % )' }} </b>
                                    </div>
                                    <div class="col-6" style="background-color: #df7626; color: white">
                                        <span class="text-120 ">  @if($invoice->currency) {{  $invoice->currency->symbol}} @endif {{number_format( round($discount ,2) )}}  </span>
                                    </div>
                                    <hr style="width: 100%"/>
                                </div>
                            @endif
                            @if($invoice->recepit->received > 0 )
                            @php $recieved = $invoice->recepit->received ;   @endphp
                                <div class="row ">
                                    <div class="col-6 text-left" style="color: #252968;">
                                        <b> Recevied   </b>
                                    </div>
                                    <div class="col-6" style="background-color: #df7626; color: white">
                                        <span class="text-120 ">  {{$invoice->currency ? $invoice->currency->symbol : ''}} {{number_format( $invoice->recepit->received , 2)}} </span>
                                    </div>
                                    <hr style="width: 100%"/>
                                </div>
                            @endif
                          @if($invoice->recepit->pending !=  round( (($total - $discount) + $vat) ,2 ))
                                 <div class="row ">
                                    <div class="col-6 text-left" style="color: #252968;">
                                         <b> Pending   </b>
                                     </div>
                                     <div class="col-6" style="background-color: #df7626; color: white">
                                        <span class="text-120 ">  {{$invoice->currency ? $invoice->currency->symbol :''}} {{ $invoice->recepit->pending > 0 ?  number_format($invoice->recepit->pending) : 0}} </span>
                                    </div>
                                    <hr style="width: 100%"/>
                                </div>
                            @endif
                            ...
                            <div class="row my-2 align-items-center bgc-primary-l3 ">
                                ...
                                <div class="col-5" style="background-color: #252968;padding: 10px; color: white">
                                    <span class=" text-success-d3 opacity-2">   @if($invoice->currency) {{ $invoice->currency->symbol}} @endif {{ number_format( round( (($total - $discount) + $vat) ,2 ) ,2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                     ...
                     <div class="row mt-4">
                      ...
                      @endif
                       <div class="col-md-4  p-3 ">
                             <h5>Yalla Repairs Building Maintenance LLC</h6>
